...
let rec get_comment (str:string) (num_open: int) (acc:string): (comment*string) = 
 match num_open with
    | 0 -> ({content=acc; sequence_num=0}, str)
    | _ -> 
     ...
     (match Str.string_match comment_regexp str 0 with
      | true -> 
       ...
       (match second_open < first_close with
        ...
        | false -> ({content=(acc^(Str.string_before str (first_close+2))); sequence_num=0}, (Str.string_after str (first_close+2))))
      | false -> 
