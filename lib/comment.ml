     ...
     (match start_comment str with
      | true -> 
       ...
       (match second_open < first_close with
        ...
        | false -> ({content=(acc^(Str.string_before str (first_close+3))); sequence_num=seq_num}, (Str.string_after str (first_close+3))))
      | false -> 
       get_comment (Str.string_after str (first_close+3)) (num_open - 1) (acc^(Str.string_before str (first_close+3))) seq_num)
