...
let rec get_comment (str:string) (num_open: int) (acc:string) (seq_num:int): (comment*string) = 
  match num_open with
    ...
    | _ -> 
     ...
     let whitespace_offset = Str.string_after str first_close |> String.lfindi ~f:(fun _ c -> not (Char.is_whitespace c)) in
     (match start_comment str, whitespace_offset with
      | true, Some w_offset -> 
       ...
       (match second_open < first_close with
        ...
        | false -> ({content=(acc^(Str.string_before str (first_close+w_offset+2))); sequence_num=seq_num}, (Str.string_after str (first_close+w_offset+2))))
      | false, Some w_offset -> 
