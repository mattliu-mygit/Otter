...
match List.zip l1 l2
   with Unequal_lengths
   -> None | Ok l ->
   Some l
;;
(* 
...
(* 
  * Now let us
   regularize this with
   a monad
  * Think of
   a monad as a wrapper
   on regular
   computations
  * "In
   Monad-land" here is
...
(* 
 * OK now let us
   redo the zip example
   with bind (using the
   macro version) 
 *
   This code looks more
   readable than the
   original, right?? 

   *)
(* "return to the monad" - here that means wrap in
   Some(..) *)
(* 
 * Here is what
   Option.return above
...
(* To show this let
   ...
   why this is what it
   is meaning *)
(* Note let%map is the let% analog of |>> which just wraps result in
   return  *)
(* 
  OK it is
   finally time for a
...
;;
(* Core requires
   that a map operation
   be defined 
       -
   map is like bind but
   the f is just a
   normal-land function
